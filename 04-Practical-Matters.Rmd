...
```{r}
probs <- data.frame(age = c(17:26), probs) %>% 
  gather(key = "Mode", value = "Probability", -age)
```
...
1. In the example in this chapter we estimated the probabilities of choosing different modes by age setting travel time to the in-sample median. Calculate the probability of choosing the modes but now as a function of **travel time**, for ages 17, 20, 23, and 26.
