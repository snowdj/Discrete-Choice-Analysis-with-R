...
Is it possible to fully specify the model and use a multinomial logit model instead?
```{r}
H_f <- H

H_f$fuel <- H$region
H_f$fuel <- ifelse(str_detect(attributes(H_f$fuel)$names, "g"), "Gas", "Electric")

H_f$setup <- H$region
H_f$setup <- ifelse(str_detect(attributes(H_f$fuel)$names, "r"), "Room", "Central")
```


```{r}
mod4 <- mlogit(depvar ~ ic + oc + fuel, H_f, reflevel = "ec")
summary(mod4)
```
